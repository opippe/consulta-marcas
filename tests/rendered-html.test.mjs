...
test("server-renders the 55 Marcas landing page", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>55 Marcas \| Registro de marcas<\/title>/i);
...
test("server-renders the results route without a previous consultation", async () => {
  ...
  assert.match(html, /Voltar para a 55 Marcas/i);
});
